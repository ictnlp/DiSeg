...
class SpeechToTextDataset(FairseqDataset):
    ...
    def collater(self, samples: List[Tuple[int, torch.Tensor, torch.Tensor]]) -> Dict:
        ...
        if self.src_texts is not None:
            # for _, _, _, t in samples:
            # if isinstance(t,list): print(t)
            ...
            transcription_lengths = torch.tensor(
                [len(t.split(' ')) if len(t.split(' ')) else 1 for _, _, _, t in samples], dtype=torch.long
            ).index_select(0, order)
            """
            transcription_lengths = torch.tensor(
            ...
            for _, _, _, t in samples:
                print(t)
                print(torch.tensor([ int(sub.startswith('▁')) for sub in self.tgt_dict.string(t)],dtype=torch.long))
            """
            """
            is_sub = torch.tensor(
                [[ sub.startswith('▁') for sub in self.tgt_dict.string(t)] for _, _, _, t in samples], dtype=torch.long
            ).index_select(0, order)
            """
            ...
            is_sub = fairseq_data_utils.collate_tokens(
                [
                    torch.tensor(
                        [
                            int(sub.startswith("▁"))
                            for sub in self.tgt_dict.string(t).split(" ")
                        ],
                        dtype=torch.long,
                    )
                    for _, _, _, t in samples
