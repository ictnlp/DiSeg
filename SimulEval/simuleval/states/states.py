...
class Entry(object):
    ...
    def update(self, new_value):
        if self.new_value_type:
            assert isinstance(
                new_value, self.new_value_type
            ), f"{new_value}, {self.new_value_type}"
...
class BaseStates(object):
    def __init__(self, args, client, instance_id, agent):
        ...
        self.test_threshold = getattr(args, "test_threshold", None)
        self.test_waitk_lagging = getattr(args, "test_waitk_lagging", None)
        self.agent = agent
        self.init_entries()
    ...
    def update_source_segment(self, num_segment=1, extra_frame=0):
        # Read a segment from server
        info = self.get_info_from_server(num_segment, extra_frame=extra_frame)
        segment = info["segment"]

        self.segments.source.append(segment)

        if info.get("finish", False) is True or segment in [DEFAULT_EOS]:
    ...
    def update_source(self, num_segment=1):
        ...
        if (
            self.test_threshold is not None or self.test_waitk_lagging is not None
        ) and len(self.units.source) == 0:
            extra_frame = 20
        else:
            extra_frame = 0

        if len(self.units.source) == 0:
            extra_frame = 5

        # If unit queue is empty, try to update a segment from server
        if self.unit_queue.source.empty():
            self.update_source_segment(num_segment, extra_frame=extra_frame)

        if not self.unit_queue.source.empty():
            # pdb.set_trace()
            self.units.source.append(self.unit_queue.source.pop())
