...
class Scorer(object):
    def __init__(self, args):
        ...
        if self.data_type == "speech" and self.eval_latency_unit == "char":
            logger.error(
        ...
        if self.data_type == "text":
            ...
            if self.data_type is None:
                logger.error("Please specify the data type (text or speech).\n")
            else:
                logger.error(
    ...
    def get_info(self):
        return {"num_sentences": len(self), "data_type": self.data_type}
    ...
    def reset(self):
        ...
        for i, (src, tgt) in enumerate(zip(self.data["src"], self.data["tgt"])):
            self.instances[i] = self.instance_class(i, src, tgt, self.eval_latency_unit)
