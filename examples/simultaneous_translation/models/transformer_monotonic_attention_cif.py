...
@register_model("transformer_unidirectional_cif")
...
@register_model("transformer_monotonic_cif")
...
class TransformerMonotonicDecoder(TransformerDecoder):
    ...
    def extract_features(
        ...
        # print("fires shape", encoder_out["fires"][0].size()[0])
        # print("fires", encoder_out["fires"][0])
        ...
        for i, layer in enumerate(self.layers):
            ...
            #     if_online = incremental_state["online"]["only"] # tensor(True)
            ...
            #         )
            #         # curr_steps = [[1, 1, 1, 1]]
            #         # (p_choose < 0.5).t().type_as(curr_steps) = [[1, 1, 1, 1]]
            #         # new_steps = [[1, 1, 1, 1]]
            ...
            #         # if (new_steps >= src).any() or encoder_out["fires"][0][-1] < 1:
